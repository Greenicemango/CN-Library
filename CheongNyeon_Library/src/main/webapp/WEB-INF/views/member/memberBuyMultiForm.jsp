...
<article>
<br><br><br>
<form method="post" name="joinForm" style="text-align:center;" action=memberCartBuy>
...
<%-- 	<c:forEach var="a" item="${name}"> --%>
	<c:forEach items="${cvo}" var="cvo" varStatus="status">
	<div id="${status.index}"><!-- 구매하고자 하는 책의 내용이 담긴 div -->
		<table class="nbuyform" width="650" cellspacing="1" border="1" >
			<tr>
				<td width="150" rowspan="5">
					<div>
						<img alt="" src="/images/book/${cvo.image}" style="max-width:100%; padding:0px;">
					</div>
				</td>
			</tr>
			<tr>
			...
			<tr>
				<td style="height:25%; border-right:none;">
					...
					value="${cvo.quantity}" style="width:40px; height:20px;" maxlength="2" readonly/>
				</td>
			</tr>
			<tr>
				<td style="height:25%; border:none;">
				<input type="hidden" name="price" value="${cvo.price}"/>
					가격 : <input type="text" name="price_cal" size="7" value="${cvo.price*cvo.quantity}" readonly/> 원
				</td>
			</tr>
			<tr>
				<td style="height:25%; border-right:none; border-left:none; border-bottom:none;">
					적립 예정 포인트 : <input type="text" name="point_cal" size="7" value="${cvo.price*0.05}" readonly/> P
				</td>
			</tr>
...
<%-- 	</c:forEach> --%>
	...
	<div id="joinInfo">
		...
		<tr>
			<td>결제 금액 : <input type="text" name="price_total" size="7" value="${totalPrice}" style="border:none; outline: none;" readonly/> 원</td>
		</tr>
		<tr>
			<td>구매 후 PAY : <input type="text" name="price_after" size="7" value="${payMoney - totalPrice}" style="border:none; outline: none;" readonly/></td>
		</tr>
		<tr>
