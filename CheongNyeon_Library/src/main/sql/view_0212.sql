...
SELECT * FROM
	(SELECT rownum AS rn, best.* FROM
		((SELECT * FROM bookproduct WHERE useyn='y' AND bestyn='y' ORDER BY indate DESC) best)
	WHERE rownum <= 21);
...
AS bname, c.quantity, b.price, c.result, c.indate, b.useyn
